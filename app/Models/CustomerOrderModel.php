...
class CustomerOrderModel extends Model
{
    ...
    protected $allowedFields = ['user_id', 'order_number', 'gateway_order_id','tax','shipping_charge',
    'order_id','shipment_id','shipping_status','awb_code','courier_company_id','courier_name',
    'discount', 'coupen_code_id','address_id','shipping_address', 'sub_total', 'total_amount', 'payment_method', 'payment_status', 'status', 'created_at'];
    public function salesHistory($searchInput=false,$filter=false,$startDate=false,$endDate=false,$orderId=false) {
        $builder = $this->db->table('customer_orders as co')
                    ->select('co.id as orderId,co.order_number,co.tax,co.shipping_charge,co.discount,co.address_id,co.shipping_address,co.sub_total,co.total_amount,co.payment_method,co.payment_status,co.status,co.created_at as orderDate,
                    pm.id as productId,
                    pm.product_title,
                    p.sku,
                    p.current_stock,
